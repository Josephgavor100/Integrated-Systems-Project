...
reflecting the real West African Power Pool (WAPP), so the network has genuine
international structure like the aviation network did.
"""
import csv
import math
import random
...
utilities = [
    ...
    (2, "Northern Electricity Distribution Company",
     "NEDCo", "NED", "Distribution", "Ghana", "Y"),
    (3, "Ghana Grid Company", "GRIDCo", "GRD", "Transmission", "Ghana", "Y"),
    (4, "Volta River Authority", "VRA", "VRA", "Generation", "Ghana", "Y"),
    (5, "Compagnie Ivoirienne d'Electricite", "CIE",
     "CIE", "Distribution", "Cote d'Ivoire", "Y"),
    (6, "Communaute Electrique du Benin", "CEB",
     "CEB", "Transmission", "Togo/Benin", "Y"),
    (7, "Societe Beninoise d'Energie Electrique",
     "SBEE", "SBE", "Distribution", "Benin", "Y"),
...
ghana_regions = {
    "Greater Accra": [
        ("Achimota", 5.614, -0.224), ("Tema",
                                      5.669, -0.017), ("Mallam", 5.560, -0.298),
        ("Legon", 5.650, -0.186), ("Kaneshie", 5.560, -
                                   0.238), ("Aboadze Junction", 5.590, -0.150),
    ],
    "Ashanti": [
        ("Kumasi Central", 6.688, -1.624), ("Ejisu",
                                            6.719, -1.463), ("Obuasi", 6.202, -1.663),
        ("Mampong", 7.062, -1.400), ("Konongo", 6.618, -1.219),
    ],
    "Western": [
        ("Takoradi", 4.895, -1.759), ("Aboadze",
                                      4.985, -1.766), ("Tarkwa", 5.301, -1.994),
        ("Axim", 4.867, -2.241),
    ],
    "Central": [
        ("Cape Coast", 5.106, -1.246), ("Winneba",
                                        5.352, -0.622), ("Kasoa", 5.533, -0.416),
        ("Assin Fosu", 5.699, -1.492),
    ],
    "Eastern": [
        ("Koforidua", 6.094, -0.259), ("Akosombo",
                                       6.300, 0.055), ("Nkawkaw", 6.550, -0.767),
        ("Suhum", 6.041, -0.451),
    ],
    "Volta": [
    ...
    "Bono": [
        ("Sunyani", 7.339, -2.326), ("Techiman",
                                     7.590, -1.938), ("Berekum", 7.453, -2.585),
    ],
    "Northern": [
        ("Tamale", 9.403, -0.842), ("Yendi",
                                    9.442, -0.011), ("Savelugu", 9.625, -0.826),
    ],
    "Upper East": [
...
for region, places in ghana_regions.items():
    for name, lat, lon in places:
        ...
        capacity = round(random.uniform(15, 400) if sub_type !=
                         "Distribution" else random.uniform(5, 60), 1)
        status = "Active" if random.random() > 0.05 else "Inactive"
        substations.append([
            sid, f"{name} Substation", name, region, "Ghana",
            round(lat + random.uniform(-0.01, 0.01),
                  4), round(lon + random.uniform(-0.01, 0.01), 4),
        ...
        for b in ids_in_region[i + 1:]:
            if random.random() < 0.55:
                ...
                dist = round(haversine_km(
                    sub_a[5], sub_a[6], sub_b[5], sub_b[6]) * random.uniform(1.05, 1.3), 1)
...
region_hub = {r: name_to_id[places[0][0]]
              for r, places in ghana_regions.items()}
...
for i in range(len(region_names) - 1):
    ...
    sub_a, sub_b = sub_by_id[a], sub_by_id[b]
    dist = round(haversine_km(
        sub_a[5], sub_a[6], sub_b[5], sub_b[6]) * 1.15, 1)
    lines.append([
...
with open("utilities.csv", "w", newline="") as f:
    w = csv.writer(f)
    w.writerow(["Utility ID", "Name", "Alias",
               "Code", "Type", "Country", "Active"])
